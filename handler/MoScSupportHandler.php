...
class MoScSupportHandler {
    static function moScSendSupportQuery($post_array) {
        ...
        if(!is_null($response) && $response != false){
            ...
            MoScUtils::mo_sc_show_error_message('There was an error while submitting your response. If the issue persists, please send us your query at samlsupport@xecurify.com');
        }
    }

    static function mo_sc_validate_contact_us_fields($post_array) {
        ...
        else if (!filter_var($post_array[MoScSupportOptionsConstants::EMAIL], FILTER_VALIDATE_EMAIL)) {
